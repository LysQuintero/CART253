...
final color BACKGROUND_COLOR = color(150, 250, 150);
//this indicates the stroke around the circle shapes , same as the background
final color STROKE_COLOR = color(00, 255, 150);
...
void draw() {
  ...
  // what it does is that when the position of the mouse moves near the radius of the Circle it will activate the color.
    if (dist(mouseX, mouseY, circleX, circleY) < CIRCLE_SIZE/4) {
  ...
  if (circleX + CIRCLE_SIZE/2 > width || circleX - CIRCLE_SIZE/4 < 0) {
  ...
  if (circleY + CIRCLE_SIZE/2 > height || circleY - CIRCLE_SIZE/4 < 0) {
    circleVY = -circleVY;
  }
}
